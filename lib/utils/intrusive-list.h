...
namespace ct::intrusive { // this code was taken from intrusive prac, interface
                          // is a little smaller, i cut off unnecessary stuff

class DefaultTag;

template <typename T, typename Tag = DefaultTag> class List {
  static_assert(std::is_base_of_v<ListElement<Tag>, T>,
                "T must derive from ListElement");

private:
  template <bool isConst> class IntrusiveListIterator {
  public:
    ...
    reference operator*() const noexcept { return *as_t_ptr(ref_node); }

    pointer operator->() const noexcept { return as_t_ptr(ref_node); }
    ...
    friend bool operator==(const IntrusiveListIterator &lhs,
                           const IntrusiveListIterator &rhs) noexcept = default;
...
public:
  ...
  // O(1)
  ConstIterator begin() const noexcept { return ConstIterator{_sentinel.next}; }

  // O(1)
  Iterator end() noexcept { return Iterator{std::addressof(_sentinel)}; }
...
private:
  ...
  static node_t &as_node(T &element) { return static_cast<node_t &>(element); }
