...
bool Mutex::MutexAwaiter::await_ready() noexcept { return enqueue_if_empty(); }

bool Mutex::MutexAwaiter::await_suspend(
    std::coroutine_handle<coro::Coroutine::promise_type> handle) {
...
Mutex::MutexAwaiter::MutexAwaiter(const Mutex &mutex) noexcept : mutex(mutex) {}
...
// mutex methods impl
Mutex::Mutex() : queue(new MutexAwaiter(*this)), gc(queue.get_dummy()) {}

bool Mutex::try_lock() noexcept { return MutexAwaiter{*this}.await_ready(); }
